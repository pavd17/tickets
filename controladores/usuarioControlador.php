...
class usuarioControlador extends usuarioModelo{
	...
	public function paginador_usuario_controlador($pagina,$registros,$privilegio,$id,$url,$busqueda){
        ...
        if(isset($busqueda) && $busqueda!=""){
			...
			OR usuario_apellido LIKE '%$busqueda%' OR usuario_telefono LIKE '%$busqueda%' OR usuario_email LIKE '%$busqueda%'
			OR usuario_usuario LIKE '%$busqueda%')) ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";
		}else{
			  $consulta="SELECT SQL_CALC_FOUND_ROWS * FROM usuario WHERE usuario_id!='$id'
			   ...
			   $tabla.='
			     <tr class="text-center" >
			       ...
			       <td>
			        ...
			        <td>
						<form class="FormularioAjax" action="'.SERVERURL.'ajax/
						usuarioAjax.php" method="POST" data-form="delete" autocomplete="off">
...
public function eliminar_usuario_controlador(){
	...
	/**---Comprobando el usuario admin--- */
	if($id==1){
		$alerta=[
			"Alerta"=>"simple",
			"Titulo"=>"Ocurrió un error inesperado",
			"Texto"=>"No podemos eliminar el usuario administrador del sistema",
			"Tipo"=>"error"
		];
		echo json_encode($alerta);
		exit();
	...
	if($check_usuario->rowCount()<=0){
		...
		/**---Comproban Tickets--- */
	$check_prestamos=mainModel::ejecutar_consulta_simple("SELECT usuario_id FROM ticket WHERE usuario_id='$id' LIMIT 1");

	if($check_prestamos->rowCount()>0){
		$alerta=[
			"Alerta"=>"simple",
			"Titulo"=>"Ocurrió un error inesperado",
			"Texto"=>"No se puede eliminar este usuario debido que tiene tickets asociados, deshabilite la cuenta,si no sera nuevamente usado",
			"Tipo"=>"error"
		];
		echo json_encode($alerta);
		exit();
		...
		if($_SESSION['privilegio_spm']!=1){
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
				"Texto"=>"No tiene permisos Perro, para realizar esta operación",
				"Tipo"=>"error"
			];
			echo json_encode($alerta);
			exit();
	...
	protected function actualizar_usuario_controlador(){
		...
		if($check_dni->rowCount()>0){
				...
				if(filter_var($email,FILTER_VALIDATE_EMAIL)){
					...
					if($check_email->rowCount()>0){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"El nuevo correo ingresado ya se encuentra en el sistema",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();
				...
				if($_POST['usuario_clave_nueva_1']!=$_POST['usuario_clave_nueva_2']){
					...
					if(mainModel::verificar_datos("[a-zA-Z0-9$@.-]{7,100}",$_POST['usuario_clave_nueva_1']) || mainModel::verificar_datos("[a-zA-Z0-9$@.-]{7,100}",$_POST['usuario_clave_nueva_2']) ){
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"Las nuevas claves no completan el formato solicitado",
							"Tipo"=>"error"
						];
						echo json_encode($alerta);
						exit();
			...
			if($check_cuenta->rowCount()<=0){
				$alerta=[
					"Alerta"=>"simple",
					"Titulo"=>"Ocurrió un error inesperado",
					"Texto"=>"Nombre y clave de administrador no son validos",
					"Tipo"=>"error"
				];
				echo json_encode($alerta);
				exit();
			...
			if(usuarioModelo::actualizar_usuario_modelo($datos_usuario_up)){
				$alerta=[
					...
					"Texto"=>"Los datos han sido actualizados con exitos",
					"Tipo"=>"success"
				];
			}else{
				$alerta=[
	...
	/*--------- Controlador paginar list prueba jesus ---------*/
	public function paginador_list_controlador($pagina,$registros,$privilegio,$id,$url,$busqueda){
        ...
        if(isset($busqueda) && $busqueda!=""){
			$consulta="SELECT SQL_CALC_FOUND_ROWS * FROM item WHERE ((item_id!='$id'
			AND item_id!='1') AND (item_codigo LIKE '%$busqueda%' OR item_nombre LIKE '%$busqueda%' 
			OR item_stock LIKE '%$busqueda%' OR item_estado LIKE '%$busqueda%' OR item_detalle LIKE '%$busqueda%'
			)) ORDER BY item_nombre	 ASC LIMIT $inicio,$registros";
		}else{
			  $consulta="SELECT SQL_CALC_FOUND_ROWS * FROM item WHERE item_id!='$id'
			  AND item_id!='1' ORDER BY item_nombre ASC LIMIT $inicio,$registros";
		}
		$conexion = mainModel::conectar();
		$datos = $conexion->query($consulta);
		$datos = $datos->fetchAll();
		   ...
		   foreach($datos as $rows){
			   $tabla.='
			     <tr class="text-center" >
			       ...
			       <td>
				       <a href="'.SERVERURL.'user-update/'.mainModel::encryption($rows['item_id']).'/" class="btn btn-success">
						     <i class="fas fa-sync-alt"></i>	
				       </a>
			        </td>
			        <td>
						<form class="FormularioAjax" action="'.SERVERURL.'ajax/
						usuarioAjax.php" method="POST" data-form="delete" autocomplete="off">
							<input type="hidden" name="item_id" value="'.mainModel::encryption($rows['item_id']).'">
					         <button type="submit" class="btn btn-warning">
							      <i class="far fa-trash-alt"></i>
					    </button>
				   </form>
